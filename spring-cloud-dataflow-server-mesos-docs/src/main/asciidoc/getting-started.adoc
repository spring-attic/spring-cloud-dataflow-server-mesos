...
+
We are using the Marathon endpoint URL of https://m1.dcos/service/marathon[https://m1.dcos/service/marathon] for this document. 
+
...
+
```
curl -X POST https://m1.dcos/service/marathon/v2/apps -d @mysql.json -H "Content-type: application/json"
```
+
...
+
```
curl -X POST https://m1.dcos/service/marathon/v2/apps -d @rabbitmq.json -H "Content-type: application/json"
```
+
...
+
```
curl -X POST https://m1.dcos/service/marathon/v2/apps -d @redis.json -H "Content-type: application/json"
```
+
...
+
```
    "MESOS_MARATHON_URI": "https://m1.dcos/service/marathon",
    "MESOS_CHRONOS_URI": "https://m1.dcos/service/chronos",
```
+
...
[source,subs="attributes"]
----
curl -X POST https://m1.dcos/service/marathon/v2/apps -d @scdf-server.json -H "Content-type: application/json"
----
+
...
+
```
dataflow:>dataflow config server --uri https://192.168.65.111:20043
```
+
...
+
```
dataflow:>app import --uri https://bit.ly/stream-applications-rabbit-docker
```
+
